...
list_del(struct List *list) {
    // LAB 6: Your code here.
    // if ((list->next == list->prev) && (list->next == list))
    //     list = list->next = list->prev = NULL;
    if (list) {
        struct List *lp = list->prev;
        struct List *ln = list->next;
        lp->next = ln;
        ln->prev = lp;
        list_init(list);
...
alloc_child(struct Page *parent, bool right) {
    ...
    if (right){
        parent->right = child;
        parent->right->refc = parent->refc ? 1 : 0;
        parent->right->right = parent->right->left = NULL;
        parent->right->parent = parent;
        parent->right->addr = parent->addr + (1ULL << (parent->class - 1));
        parent->right->class = parent->class - 1;
    } else {
        parent->left = child;
        parent->left->refc = parent->refc ? 1 : 0;
        parent->left->right = parent->left->left = NULL;
        parent->left->parent = parent;
        parent->left->addr = parent->addr;
        parent->left->class = parent->class - 1;
    }
...
attach_region(uintptr_t start, uintptr_t end, enum PageState type) {
    ...
    while (step < end){
        uintptr_t size = end - step;
        cls = 0;
        while ((size >= (CLASS_SIZE(cls))) && !(step & CLASS_MASK(cls))){
            cls++;
        }
        cls--;
        if (cls == -1)
...
dump_virtual_tree(struct Page *node, int class) {
    ...
    if ((node->state & NODE_TYPE_MASK) == MAPPING_NODE){
        cprintf("MAPPING NODE phys: %p  class %d\n", (void *)PTE_ADDR(node->phy->addr), class);
    }
    if (node->right)
...
dump_memory_lists(void) {
    ...
    for (int i = 0; i <= MAX_CLASS; i++){
        ...
        uint64_t num = (max_mem_addr - min_mem_addr) / cur_size;
        if ((max_mem_addr - min_mem_addr) % cur_size){
            num = 1;
        }
        for (int j = 0; j < num; j++){
...
dump_page_table(pte_t *pml4) {
    ...
    for (size_t i = NUSERPML4; i < PML4_ENTRY_COUNT; i++){
        pdpe_t *pdpt = (pdpe_t *)KADDR(PTE_ADDR(pml4[i]));
        for (size_t j = 0; j < PDP_ENTRY_COUNT; j++){
            //pde_t *pde = (pde_t *)KADDR(PTE_ADDR(pdpt[j]));
            if (pdpt[j] & PTE_PS){
                dump_entry(pdpt[j], 1*GB, 0);
            }
            pde_t *pde = (pde_t *)KADDR(PTE_ADDR(pdpt[j]));
            for (size_t q = 0; q < PD_ENTRY_COUNT; q++){
                if(pde[q] & PTE_PS){
                    dump_entry(pde[q], 2*MB, 0);
                }
                pte_t *pte = (pte_t *)KADDR(PTE_ADDR(pde[q]));
                for (size_t l = 0; l < PT_ENTRY_COUNT; l++){
                    if(pte[l] & PTE_PS){
                        dump_entry(pte[l], 4*KB, 0);
                    }
                }
            }
        }
...
map_page(struct AddressSpace *spc, uintptr_t addr, struct Page *page, int flags) {
    ...
    if(!(pd[pdi0] & PTE_P) && alloc_pt(pd + pdi0) < 0) return -E_NO_MEM;
        ...
        pte_t *pt1 = KADDR(PTE_ADDR(pd[pdi0]));
        if (alloc_fill_pt(pt1, old & ~PTE_PS, 4 * KB, 0, PT_ENTRY_COUNT) < 0) return - E_NO_MEM;
    }
...
switch_address_space(struct AddressSpace *space) {
    ...
    // old.cr3 = current_space->cr3;
    // old.pml4 = current_space->pml4;
    // old.root = current_space->root;
    // current_space->root = space->root;
    // current_space->cr3 = space->cr3;
    // current_space->pml4 = space->pml4;
}
...
init_memory(void) {
    ...
    if (map_physical_region(&kspace, KERN_BASE_ADDR, 0, max_memory_map_addr, PROT_W | PROT_R | ALLOC_WEAK) < 0){
    ...
    if (map_physical_region(&kspace, (uintptr_t)(KERN_STACK_TOP - KERN_STACK_SIZE), PADDR(bootstack), (size_t)KERN_STACK_SIZE, PROT_W | PROT_R) < 0){
        panic("Can't map phys region start: %p of size: %lu", (void *)PADDR(bootstack), (size_t)(KERN_STACK_SIZE));
    }
    if (map_physical_region(&kspace, (uintptr_t)(KERN_PF_STACK_TOP - KERN_PF_STACK_SIZE), PADDR(pfstack), (size_t)KERN_PF_STACK_SIZE, PROT_W | PROT_R) < 0){
    ...
    for (; mstart < mend; mstart = (void *)((uint8_t *)mstart + uefi_lp->MemoryMapDescriptorSize)) {
        if (mstart->Attribute & EFI_MEMORY_RUNTIME) {
            ...
            if (map_physical_region(&kspace, (uintptr_t)mstart->VirtualStart, (uintptr_t)mstart->PhysicalStart, (size_t)mstart->NumberOfPages*PAGE_SIZE, PROT_W | PROT_R) < 0){
                panic("Can't map phys region start: %p of size: %llu", (void *)mstart->PhysicalStart, (size_t)mstart->NumberOfPages*PAGE_SIZE);
            }
        }
    }
